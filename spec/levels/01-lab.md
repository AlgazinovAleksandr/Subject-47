...
- ⭐⭐ **2026-09-24 (the user's call): the nook panel glows faintly, but ONLY while you
  stand inside BreakerNook.** The user: *"When you enter the right room, it's a bit unobvious how to
  find the light switcher there… just a slight shine so that you can see it in the dark, but only
  when you get to the final room."* `Breaker.set_glow(k)` puts the panel's own art on as an
  `emission_texture` at `Breaker.GLOW_MAX` **0.025** × `k`; `level_1.gd:_tick_nook_glow()` fades `k` in over `NOOK_GLOW_FADE`
  (1.5 s) while the player stands inside BreakerNook's own room rectangle, with no margin, so it
  never starts from Shaft's side of the doorway, and back to 0 when they leave; the throw snaps it to 0 with the
  beacons, and a restored (already-thrown) breaker never glows. No light node, no pulse. It
  narrows "no breaker self-illuminates" and "the beacon and the meter remain the only ways to find
  it" to everywhere OUTSIDE the 6 × 4 m nook, so the 9.4 m leak of Issues 62/63 is not reopened.
  Guards: `check_nook_dark.gd` (still dark at spawn and from Shaft;
  glowing in the nook but never above the cap; fades out on leaving; dark the instant it is thrown) and a
  torch-locked in-nook capture in `screenshot_nook_panel.gd`, measured as contrast against the wall.
...
⚠️ **The nook glow's strength was picked from rendered frames (2026-09-24), not typed in.**
Torch locked, `screenshot_nook_panel.gd`'s NOOK_POSES rows, panel vs a ring of wall around it:
**Hand-played and approved by the user the same day** (*"Looks fine for me"*; the
nook breaker was their third, thrown ~21 s after the torch locked at the wing entrance, 0 deaths).
`GLOW_MAX` **0.06** gave panel mean 0.27 / max 0.35 luminance and read as a solid grey box, brighter
than the wing's own dark-red doorway strips (it was the brightest thing in the wing, i.e. Issue 33
again); **0.012** (mean 0.11) kept the box but lost the fuse art; **0.025** (mean ~0.17, max ~0.22,
≈ 55/255 over the black wall at 3–5 m) reads as a dim fuse box on the wall with its layout just
legible, at about the strips' brightness. From Shaft (4.4 m, one room out) the glow is 0 and the
panel adds nothing over the wall. Rejected: glowing only the pilot lamp (a 4 cm dot is too small to
find in a 6 × 4 m room — the user chose the whole panel), a pulse, and a light node (a light would
illuminate the walls and turn the room into a lamp). The glow is gated by the ROOM rect with no
margin, not by distance, because distance leaks through the doorway into Shaft. `set_glow(0)`
disables emission outright, so at spawn the material is byte-for-byte the pre-change one and
`check_nook_dark`'s original thirteen checks still apply. ⚠️ `check_nook_dark.gd` reads
`GLOW_MAX` off the live instance's script: naming `Breaker` in a SceneTree test compiles
`breaker.gd` before the autoloads exist (it references `GameState`) and the whole test fails.
