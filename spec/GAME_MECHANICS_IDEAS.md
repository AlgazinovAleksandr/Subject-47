...
| **Stop-delayed footstep echo** | SCARY P2 | **PARTIAL → sustained version BUILT in the Void's loop corridor (2026-09-20 pass 2: from lap 3, shared `footstep` 0.35 s late from 2 m behind while walking, `level_3.gd`)** | `enable_footstep_echo()` `player.gd:423-432` fires exactly one phantom step (`:244-246`, `:251-253`); no `trail_steps`, nothing on `_is_moving` going false. One caller: `backrooms.gd:111`. | XS |
| **Sanity-effect suite (diegetic UI)** | REPORT #2 → SCARY P8 | **PARTIAL → 4 of 6 effects BUILT in the Void (2026-09-20)** | `panic_hud.gd:lie(ratio, seconds)` is the panic-spike lie (effect 1) — a shader override the player's per-frame drive respects, real `_panic` never moves; `void_stare_director.gd` fires it plus a forced BLINK with a one-frame second figure (a P3 flash), a `ScreenText.scrawl` (effect 6's register) and the nearest lamp dimmed to 20 % (effect 5's shape), driven by **continuous stare time at a fractured figure** rather than the panic ratio, with `ApparitionDirector`'s gates. ⚠️ Void only; effects 2–4 (objective / carried / journal lies) still unbuilt; **defect §3(d) still caps the tint at 10 %**, so the lie registers through the blur. | S |
...
**2026-09-20 — THE VOID (backlog `08-void.md`, the grill after the six-capture playtest)**

| Date | Idea | Reason |
|---|---|---|
| 2026-09-20 | **Restage stalkers B and D so their movement can be witnessed** (B first seen at 2.6 m, D at 1.8 m from the Morgue landing pad → look-away windows of 1.1 s and 0.44 s; the run's one death was D at 244.3 s, 2.8 s after entering) | ⚠️ **DECLINED BY THE USER AFTER BEING SHOWN THE NUMBERS — "Leave both."** The near-instant threat is the design; the rule is taught by dying once. The measurements stand (Issue 222 records them); the recommendation does not. Do not re-pitch a witness window for B or D. |
| 2026-09-20 | **Reused art as "the Void is made of the places you have been"** — the intro's gurney pad, the Lab's monitor face, the House's crayon drawing on the Void's fragments | **REJECTED TWICE, in the user's words.** 2026-09-19 capture #2: *"These beds (objects are taken from the intro level). We should create custom 3d-looking objects for this level"*; 2026-09-20 capture #6: *"I see that the picture is duplicated from the house level and the monitor is duplicated from the lab level. Let's make unique objects and decorations for this level."* The earlier-level ECHO survives only in the notes' text ("Trial 1. The ward…"); every OBJECT and texture in the Void is now Void-native. |
| 2026-09-20 | **A panic drip for staring at a stalker from beyond 3 m** (the far-stare cheese: stand at 4 m and stare it into its corner for free) | Offered as the third option and not taken. The stare is paid for in **channels** — a whisper loop and the P8 hallucination ladder — never a new panic term, per §0.2's governing finding. |

**2026-09-20 — THE VOID, pass 3 (an Opus research menu, grilled; backlog `08-void.md`)**

| Date | Idea | Reason |
|---|---|---|
| 2026-09-20 | **Solve the three shapes in the inscription's order** (door, bed, window), out-of-order resets the sequence | Not taken. A refusal that could mean wrong angle OR wrong order is exactly the ambiguity that produced 20 wrong presses in the evening run (Issue 226). Carried anchors with distinct keystones were chosen instead. |
...
| Date | Idea | Reason |
|---|---|---|
| 2026-09-20 | **Anatomy's voluntary corruption index** (tapes that rewrite the space) | Studied; the structural finding kept (escalate content, then geometry, the filter last) but not built — it becomes optimisable the moment it is required. Revisit for THE RETURN. |
| 2026-09-20 | **Control's Oceanview bell** (a global index cycling near-identical rooms) | Not taken this pass; the cycling is good, its matching puzzle is spot-the-difference. Deferred. |
| 2026-09-20 | **Manifold Garden's fall-that-never-lands / Antichamber's failing forward** | Both delete the abyss as a fatal channel — a difficulty change the user did not ask for, and the fall-and-return reads as a bug. |
| 2026-09-20 | **Paratopic's stolen completion / Superliminal's solidifying light** | Deferred; the first needs an audio bridge or it is a broken trigger, the second sits beside the level's own anamorphosis. |
| 2026-09-20 | **NaissanceE's rhythmic breath, Babbdi's one-tool, Fear & Hunger's traced circles, Lorn's Lure climbing, ECHO's learning enemies** | Rejected: §8.5's shape in a rhythm hat; inventory management; mouse-drag against mouselook; platforming over a pit with a panic bar; a sixth creature system. |
| 2026-09-20 | **A "which of these is wrong" chamber in the Void** | Legal (§8.10 is Backrooms-specific) but not taken: an anomaly that looks like a z-fight is indistinguishable from a defect in a level whose premise is broken geometry, and The Exit 8's own economy admits the design survives because it ends. |
| 2026-09-20 | **A 2D reconstruction (tangram) overlay** | Not taken; the closest to the House's genre. The Hall of Frames was chosen. |
...
| Idea | Condition to revisit |
|---|---|
| **The Void's doorway that was not there** (SCARY P11's first build: on the return leg Hall2 gains a doorway onto a 3 m alcove with one chair facing the corner; one-shot, never on the route, zero cost to miss) — offered 2026-09-20 pass 3 | Not chosen over STEP THROUGH + SEARCH this pass; still legal and cheap (M). Revisit after the pass-3 playtest if the early rooms still feel thin. §8.10 audit already done: no audit, no counter, no failure cost. |
| **The child room's music box freezes every far-wing figure while it plays** — offered 2026-09-20 pass 3 | Not chosen. It hands the player a switch over the monsters; the fear document's stalker is "at its ceiling with both cheeses patched" (§8.3) and this would be a third. Revisit only as a one-shot, not a toggle. |
| **The thing that is never drawn** (SCARY P10: an `AudioStreamPlayer3D` walking the room graph ≥ 12 m away, never entering your room, never resolving) — offered 2026-09-20 pass 3 for the Archive / loop corridor | Not chosen this pass — a channel, not a challenge, and the pass had two challenge slots. Zero regression surface; the cheapest unbuilt retrofit left. |
| **A fourth memory nobody counted** (a FACE assembled from the other three shapes seen end-on from the Hall2 landing pad; opens nothing; the inscription's "three places" becomes a lie by omission) — offered 2026-09-20 pass 3 | Not chosen; keep for a later Void pass. Legal under §5.5 (diegetic unreliability) and not §8.6 (the rules stay true). |
...
| **Q5. Wall-maze minigame placement** — Void climax, or inside the investigation level? | **Neither — cut it.** Placement being undecided across two sessions is the signal; the House already ships a 2D minigame with its own generator stress test. ⚠️ **Reversed knowingly by the user on 2026-09-20 (Void pass 4)** after the fourth playtest (*"maybe we should add another minigame somewhere?"*): the Void gained THE HALL OF FRAMES — in-world, step-through based, a different genre from the House's maze, which is the half of this reasoning that survived. |
