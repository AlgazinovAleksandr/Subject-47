...
| `intro/intro_wall.png` | Cold dark concrete/stone, peeling institutional wallpaper over damp plaster — a sharper, more detailed replacement for `wall_intro.png` | ⭐ Since 2026-09-24 (the Intake Wing) no longer on any wall — `_apply_textures()` is gone; it is the SOURCE plaster `tools/make_intro_wing_art.py` composes `asylum_wall.png` and `asylum_door.png` from | done |
| `intro/floor_intro.png` | Dark stone slab — faint cracks, slightly uneven | ⭐ Since 2026-09-24: the observation HALL's floor only (its per-room skin — the one room somebody mops), and the source of `asylum_floor.png` | done |
| `intro/ceiling_intro.png` | Rough concrete ceiling — darker than walls, slight water stain | ⭐ Since 2026-09-24: the source of `asylum_ceiling.png`; no longer applied directly | done |
...
| `intro/asylum_wall.png` | ⭐ The Intake Wing (2026-09-24). Peeling grey-green plaster over a DARK painted wainscot, one tile = 3.6 m square, floor at the bottom edge, seamless in x. **Composed, not generated** (the flux quota was exhausted that day): `intro_wall.png` graded cooler, the bottom 1.0 m re-painted dark with a ragged dado line, floor grime, a faint tide line — `tools/make_intro_wing_art.py`. Flux prompt for a replacement: *"flat straight-on texture of a derelict asylum wall, peeling pale grey-green plaster over a dark slate-green painted lower wainscot band, cracks, damp stains, even soft light, seamless"* | Every wall of the wing (RoomBuilder `wall_mat`, **world-space** triplanar, scale 1/3.6, positive V — `intro_room.gd:_build_room()`), the door infills and the window sill/head | done |
| `intro/asylum_floor.png` + `asylum_floor_rough.png` | Wet stained concrete, seamless both ways, + its roughness map (wet patches 0.25, dry 0.85 — the bulbs glint in the puddles). Composed from `floor_intro.png` with FFT-lowpassed stain fields. Flux prompt: *"top-down texture of a wet, stained concrete floor in an abandoned hospital, dark grey, puddle sheen, rust and grime, even light, seamless"* | Every floor of the wing except the hall's | done |
| `intro/asylum_ceiling.png` | Cracked plaster ceiling with soft damp stains, each with one darker tide line. Composed from `ceiling_intro.png`. Flux prompt: *"looking straight up at a cracked water-stained plaster ceiling, grey, brown damp rings, even light, seamless"* | Every ceiling of the wing | done |
| `intro/asylum_door.png` | A panelled ward door (537×1024 = the 1.10 × 2.10 m leaf), pale grey-green peeling paint, wired-glass light, escutcheon + knob, edge wear. Composed from `intro_wall.png`'s plaster + drawn panels. Flux prompt: *"straight-on photo of an old wooden asylum ward door, pale grey-green peeling paint, small wire-glass window, filling the frame"* | Every `WingDoor` leaf (both faces), incl. the five locked corridor doors | done |
| `intro/tally_marks.png` | RGBA cutout: **46** tally marks (nine gates of five and one), three passes of a fingernail each | The cell's west wall beside the bed, 0.9 × 0.45 m quad | done |
| `intro/reel_panel.png` | The reel-to-reel's front panel: brushed aluminium, two VU meters, piano keys, knobs, and a masking-tape label **SESSION 46** in marker (Pillow) | `ReelToReel` UseProp, front quad 0.36 × 0.12 | done |
| `intro/file_subject47.png` | YOUR file, open: the INTAKE — SUBJECT 47 sheet (NO PHOTO, admitted 04:12…) and Subject 46's stapled page stamped **TERMINATED** (Pillow) | `SubjectFile` note on the hall desk, 0.42 × 0.28 quad, emission 0.25 | done |
| `intro/observer_log.png` | Clipboard page: OBSERVATION LOG, 04:12 admitted … 06:58 stirring, *do NOT let it see the file.* (Pillow) | `ObserverLog` note on the hall's south wall | done |
| `intro/tray_label_issued.png` | Embossed label tape: **SUBJECT 47 — ISSUED** (Pillow) | The placard on the torch trolley's front edge | done |
| `intro/wristband_47.png` | The band's printed tag: SUBJ 47 · INTAKE 3 · ADM 04:12 (Pillow) | `WristbandNote` on the cell's bedside cabinet | done |
...
| `level_2_house/forest.png` | Moonlit treeline behind the window glass — faint, self-illuminated so it reads in the dark room | Level 2 (The House) — `WindowForest` quad built in `level_2.gd` `_spawn_window()`. ⚠️ **SUPERSEDED by the porch pass (2026-09-24)**: the painted window is being removed in favour of a real back porch and forest built from the `forest_*` / `night_sky` rows below. Since 2026-09-24 (b) nothing loads it: **untracked and gitignored**, local-only | retired |
...
| `level_2_house/plaster_hole.png` | Ragged hole knocked through old plaster: broken plaster rim and lath around a pitch-black cavity. **Real RGBA** (512×512): everything outside the broken rim is alpha 0, so it sits on `house_wall.png`; plaster graded down toward the wallpaper's brown | Level 2 — the back porch (2026-09-24 porch pass) — the hole the watermelon is hidden in. Wall prop: inset ≥ 0.16 (CLAUDE.md) | ⚠️ **Local-only since 2026-09-24 (b)**: the level loads `plaster_hole_ring.png` (this art with its painted-black centre made transparent), so this file is untracked and gitignored | retired |
| `level_2_house/guillotine_blade.png` | Old guillotine blade, straight-on front view: a two-tier rusted dark iron weight (four bolt heads, two through-holes, slits between the bars) over worn scratched dull steel with a single OBLIQUE cutting edge — **low on the LEFT, high on the right** — a paler ground bevel along it, rust blooms, and dark red-brown dried stains and short runs at the edge. **Real RGBA** 1024×576 (1.78:1): the triangle under the diagonal, the bolt holes and the bar slits are alpha 0; 5 px clear margin, 29 % clear. Built by `tools/make_guillotine_blade.py` from flux raw `porch/blade_raw_p1_a.jpg` — the weight and steel are the raw's, the oblique edge is CUT by the tool (flux would not draw one) | Level 2 — the guillotine blade in the forest stump and mounted in the guillotine (2026-09-24 d), on a QuadMesh per face | done |
