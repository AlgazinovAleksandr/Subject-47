...
The most deliberate early decision: no active enemy AI for most of the game. The creatures exist but, in Levels 1–3, never move on their own (⚠️ **written before Levels 6 and 7** — `creature_object12.gd` is a five-state pursuit AI in The Breach and `dungeon.gd` runs the Parasite hunter in waves, so the "one exception" below is now one of three; the pillar became "AI is rationed — one chase level in twelve", see `CLAUDE.md`) — horror comes from atmosphere, lighting, sound, and scripted screamer triggers, not a nav-mesh chase. This was partly pragmatic (no time to build a full chase system) but it turned out to be an interesting constraint. Static horror — something that is *there* but doesn't acknowledge you — is often more unsettling than something that chases you. You can outrun a monster; you can't outrun the feeling that it already knows where you are.
...
The level sequence tracks a psychological descent. The Lab is institutional and ordered (fluorescent lights, linoleum). The House is personal and decayed (wallpaper, wooden floors, a bedroom). The Corridor (added later as Level 3) is a haunted-hotel hallway — still architecturally coherent but stretched into an endurance gauntlet. The Void has no coherent geometry at all — corridors loop, tiles float. Each step removes one more layer of the familiar. By the time you reach the Void (Level 8 — this file says Level 4 throughout; the Void was renumbered when KONTUR, The Breach and THE NIGHTMARE shipped), spatial logic has broken down entirely, which is meant to mirror what the experiment is supposedly doing to the subject.
