...
- ⭐ **PITCH BLACK UNTIL THE POWER IS RESTORED (2026-09-03, the user's call).** `_boost_ambient`
  ...
  purpose" (the Morgue, the ten-room wing) from "currently unlit", which zeroing would erase
...
- **Scares**: random blackouts (all lamps stutter dark ~1.5 s) and pipe groans (`pipe_groan.wav`) on
  ...
  **6.75 m / about 1.7 seconds** into the level, identically, every run. It now arms at
  `randf_range(APPARITION_AT)` = 42–50 s and fires on the first poll that passes the same four
  ...
  ⚠️ `APPARITION_DEADLINE` 65 s drops those gates: the beat teaches a rule the player is killed by
...
- **(superseded) Gate 6 — THE PHONE** (`rotary_phone.gd`, *destroy* — was *ignore* until **BUG_FIX.md 4.6**): a
...
- ⭐⭐ **IT USED TO WALK THROUGH WALLS, AND SINCE 2026-09-07 IT ROUTES.** `_move_toward()` assigns
  ...
  0 crossings in 95 chase steps.** ⚠️ **Not a navmesh and not A\*** — 13 axis-aligned rooms and 14
  ...
  - ⚠️⚠️ **THE ROUTER SHIPPED WITH TWO BUGS AND `walk_level6_breach.gd` CAUGHT BOTH** — the level's
    ...
    **(b) A point ON a shared wall plane belongs to BOTH rooms.** `RoomBuilder` requires connected
    ...
    room, padded by one wall thickness* **before** asking which room the target is in.
