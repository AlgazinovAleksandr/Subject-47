...
**Intro — THE INTAKE WING** (rebuilt 2026-09-24) — six rooms on `RoomBuilder`, built at runtime by
`intro_room.gd`: cell (wake, sit up, stand — the restraints already open) → the cold-open video's corridor → observation hall (one-way
glass, torch issued) → the old 12×18 m ward (blackout, switch, note, wheelchair) → calibration (the
first time the panic bar moves) → airlock → Lab. The observer speaks in **five voice lines only** —
the user's rule; everything else is captions and paper.
...
| `intro_room.tscn` | `extra_lock` — the airlock opens after calibration | Minimal scene: root + Environment + AmbientPlayer + Player — the six-room Intake Wing is built by `intro_room.gd` on `RoomBuilder`; the player is moved onto the cell bed at wake. `ExitDoor` sits in the **airlock** (`AIRLOCK_EXIT_POS`); under `GameState.is_ending` only the ward is built and `ExitDoor` is back on its back wall. Back from the Lab, the wing is built solved with the player in the airlock |
...
- `set_panic_ceiling(ratio)` (2026-09-24, **the intro only**) — clamps panic to `PANIC_MAX × ratio` in
  `_update_panic`, `add_panic` and `set_panic_ratio`, so the screamer cannot fire from panic. Default
  1.0 = off; the Player is re-instanced per scene, so it cannot leak into another level
