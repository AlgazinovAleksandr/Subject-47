...
| `textures/level_2_house/porch/*_raw.jpg` (2026-09-24, flux-1-schnell; every prompt and which raw is USED in `prompts.txt` beside them — the rest are rejected alternatives) | `game/assets/textures/level_2_house/` `house_witch`, `forest_ghost_{woman,crawler,tall}`, `forest_pine`, `forest_bark`, `forest_floor`, `night_sky`, `watermelon_flesh`, `plaster_hole` (`watermelon_rind` is drawn, not keyed) | `tools/make_house_porch_art.py` (green-screen key incl. enclosed pockets, alpha bleed, seamless tiling, sky assembly; asserts every cutout has a clear border) | local ⚠️ — the tool READS eleven of them, so by this file's rule they should be committed, but `.gitignore`'s `assets_src/textures/**/*_raw.*` catches them (the Breach approach raws are in the same state). Needs a `!` exception per used raw, or a rename, if a fresh clone must be able to re-run the tool |
